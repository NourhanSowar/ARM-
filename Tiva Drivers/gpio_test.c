...
void GpioMode (unsigned long int port,unsigned int  pin , gpio_digital  enable , mode_t mode    ) // alternative and digital
{
   if  ( GetSelectedMode( port ,GPIOAFSEL_GPIO )  == 0   )
    {
...
{
    if  (mode == GPIOAFSEL_ALT)
    {
        return 1 ;
    }
    else if (mode == GPIOAFSEL_GPIO)
    {
        return 0 ;
    }

}
void GpioDirection (unsigned long int port,unsigned int  pin ,pin_mode direction   )  // in or out
{
